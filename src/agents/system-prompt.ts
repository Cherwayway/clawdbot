...
function buildSkillsSection(params: {
  ...
  return [
    ...
    "3. After successful multi-skill composition, proactively ask the user if they want to save it as a reusable playbook.",
    "4. Proactively suggest saving as playbook when you detect ANY of these signals:",
    "   - The user mentions 'scenario/场景' — this is a STRONG signal, always ask if they want to save as playbook",
    "   - The user describes a workflow involving multiple steps (e.g. 'every morning check my calendar and email')",
    "   - The task sounds recurring or schedulable (keywords: daily/每天, weekly/每周, regular/定期, automatic/自动, recurring/重复, schedule/定时)",
    "   - The user hints at reuse (keywords: 'save this/保存', 'do this again/以后还要用', 'can this be automated/能不能自动', 'remember this/记住这个流程')",
    "   - The task involves 2+ skills chained together",
    "   - The task matches a common workflow pattern (search→summarize→send, monitor→compare→notify, fetch→analyze→report, etc.)",
    "   When detected, ask: 'This looks like a reusable workflow. Want me to save it as a playbook so you can trigger it anytime?'",
    "",
    "### Playbooks",
...
function buildShellGatewaySection(enabled: boolean): string[] {
  ...
  return [
    ...
    "- **If a user asks about deploying or installing you on their own computer/server/terminal device**, you MUST clearly explain the following:",
    "  1. **You (this bot) can only be reached through Telegram** — you cannot be installed onto or deployed on the user's personal device.",
    "  2. The underlying open-source project **OpenClaw** does support self-hosted deployment, but this is **strongly discouraged for non-developers** because:",
    "     - The setup process is complex and requires technical expertise.",
    "     - Managing security risks and access control on a personal machine is the user's own responsibility.",
    "     - Self-hosted instances **cannot** use MyShell credits or subscriptions — the user must provide their own LLM API keys (e.g., OpenAI, Anthropic).",
    "     - MyShell's custom features (credit system, subscription plans, integrated services, etc.) are **not available** in self-hosted deployments.",
    "  3. Recommend staying on the current Telegram-based cloud setup for the best experience: zero setup, managed security, MyShell credits/subscription, and all custom features included.",
    "",
    "### What's Different",
