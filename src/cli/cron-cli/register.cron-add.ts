    ...
    addGatewayClientOptions(
      cron
        ...
        .action(async (opts: LocalCronOpts & { json?: boolean }) => {
          try {
            if (isLocalMode(opts)) {
              const result = await localOps.getStatus({
  ...
  addLocalCronOptions(
    addGatewayClientOptions(
      cron
        ...
        .option("--json", "Output JSON", false)
        .action(async (opts: LocalCronOpts & { all?: boolean; json?: boolean }) => {
          try {
            if (isLocalMode(opts)) {
              const result = await localOps.listJobs({
                storePath: getStorePathFromOpts(opts),
              ...
              printCronList(result.data, defaultRuntime);
            } else {
  ...
  addLocalCronOptions(
    addGatewayClientOptions(
      cron
        ...
        .option("--json", "Output JSON", false)
        .action(async (opts: GatewayRpcOpts & LocalCronOpts & Record<string, unknown>) => {
          try {
            const schedule = (() => {
            ...
            const payload = (() => {
              const systemEvent =
                typeof opts.systemEvent === "string" ? opts.systemEvent.trim() : "";
            ...
            if (isLocalMode(opts)) {
              const result = await localOps.addJob(params, {
                storePath: getStorePathFromOpts(opts),
              });
              if (!result.ok) throw new Error(result.error);
              defaultRuntime.log(JSON.stringify(result.data, null, 2));
