  ...
  addLocalCronOptions(
    addGatewayClientOptions(
      cron
        ...
        .action(async (id, opts: LocalCronOpts & Record<string, unknown>) => {
          try {
            ...
            if (typeof opts.agent === "string" && (opts.agent as string).trim()) {
              patch.agentId = sanitizeAgentId((opts.agent as string).trim());
            }
            if (opts.clearAgent) {
            ...
            if (opts.at) {
              ...
              patch.schedule = {
                ...
                tz:
                  typeof opts.tz === "string" && (opts.tz as string).trim()
                    ? (opts.tz as string).trim()
                    : undefined,
              };
            }
            ...
            const model =
              typeof opts.model === "string" && (opts.model as string).trim()
                ? (opts.model as string).trim()
                : undefined;
            const thinking =
              typeof opts.thinking === "string" && (opts.thinking as string).trim()
                ? (opts.thinking as string).trim()
                : undefined;
            ...
            if (hasSystemEventPatch) {
              ...
              patch.payload = payload as CronJobPatch["payload"];
            }

            if (typeof opts.postPrefix === "string") {
              patch.isolation = {
                postToMainPrefix: (opts.postPrefix as string).trim()
                  ? (opts.postPrefix as string)
                  : "Cron",
              };
            }

            if (isLocalMode(opts)) {
              const result = await localOps.updateJob(id, patch, {
