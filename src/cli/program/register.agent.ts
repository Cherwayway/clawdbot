...
export function registerAgentCommands(program: Command, args: { agentChannelOptions: string }) {
  program
    ...
    .option("--session-key <key>", "Explicit session routing key for config lookup")
    ...
    .option("--jsonl", "Stream output as JSONL (one JSON object per line for each block)", false)
    .option(
      "--timeout <seconds>",
